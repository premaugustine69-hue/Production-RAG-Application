...
from production_rag.server.utils.auth import authenticated
...
class IngestResponse(BaseModel):
    object: Literal["list"]
    model: Literal["production-grade-rag"]
...
async def ingest(request: Request, file: UploadFile) -> IngestResponse:
    ...
    """
    return await ingest_file(request, file)
...
async def ingest_file(request: Request, file: UploadFile) -> IngestResponse:
    ...
    ingested_documents = await service.async_ingest_bin_data(file.filename, file.file)
    return IngestResponse(object="list", model="production-grade-rag", data=ingested_documents)
...
async def ingest_text(request: Request, body: IngestTextBody) -> IngestResponse:
    ...
    if len(body.file_name) == 0:
        raise HTTPException(400, "No file name provided")
    ingested_documents = await service.async_ingest_text(body.file_name, body.text)
    return IngestResponse(object="list", model="production-grade-rag", data=ingested_documents)
