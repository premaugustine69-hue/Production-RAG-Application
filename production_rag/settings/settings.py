...
from production_rag.settings.settings_loader import load_active_settings
import os
...
class AuthSettings(BaseModel):
    ...
    Supports both the legacy simple-secret mode and the new JWT mode.
    When ``jwt_enabled`` is True, the platform issues JWT access/refresh tokens.
    The legacy ``secret`` field remains active for backward compatibility.
    """
    ...
    jwt_enabled: bool = Field(
        description="Enable JWT-based authentication (Bearer tokens).",
        default=False,
    )
...
class RedisSettings(BaseModel):
    ...
    password: str = Field(
        "",
        description="Redis AUTH password (empty string = no password).",
    )
    ...
    embedding_cache_ttl_days: int = Field(
        7,
        description="TTL in days for cached embedding vectors.",
    )
    session_ttl_hours: int = Field(
        8,
        description="TTL in hours for cached user sessions.",
    )
    conversation_ttl_hours: int = Field(
        2,
        description="TTL in hours for cached conversation history.",
    )
...
class S3Settings(BaseModel):
    ...
    bucket_name: str = Field(description="S3 bucket for document storage")
    region: str = Field("us-east-1", description="AWS region")
    endpoint_url: str | None = Field(None, description="Localstack/Minio URL")


class SQSSettings(BaseModel):
    """SQS queue configuration."""
    ingestion_queue_url: str = Field(description="SQS queue for async ingestion")
    region: str = Field("us-east-1", description="AWS region")
    endpoint_url: str | None = Field(None, description="Localstack/MQ URL")


class SecretsSettings(BaseModel):
    """AWS Secrets Manager configuration."""
    secret_id: str = Field(description="Secret ID containing platform credentials")
    region: str = Field("us-east-1", description="AWS region")
    enabled: bool = Field(False, description="Enable fetching secrets on startup")


class AwsSettings(BaseModel):
    """AWS unified configuration."""
    s3: S3Settings | None = None
    sqs: SQSSettings | None = None
...
class ServerSettings(BaseModel):
    ...
    port: int = Field(description="Port of Production Grade RAG Application FastAPI server, defaults to 8001")
...
class MilvusSettings(BaseModel):
    uri: str = Field(
        "local_data/production_rag/milvus/milvus_local.db",
        description="The URI of the Milvus instance. For example: 'local_data/production_rag/milvus/milvus_local.db' for Milvus Lite.",
    )
    token: str = Field(
        "",
        description=(
...
# Phase 6: AWS Secrets Manager injection
aws_cfg = unsafe_settings.get("aws", {})
secrets_cfg = aws_cfg.get("secrets", {})
if secrets_cfg.get("enabled"):
    from production_rag.components.secrets.secrets_manager import load_secrets_to_env
    load_secrets_to_env(
        secret_id=secrets_cfg.get("secret_id"),
        region_name=secrets_cfg.get("region", "us-east-1"),
    )
    # Merge env vars back into unsafe_settings for the sections that might use secrets
    for section in ["postgres", "redis", "server"]:
        if section in unsafe_settings and isinstance(unsafe_settings[section], dict):
            for k, v in unsafe_settings[section].items():
                env_key = f"{section.upper()}_{k.upper()}"
                if env_key in os.environ:
                    unsafe_settings[section][k] = os.environ[env_key]
            # Also handle nested auth
            if section == "server" and "auth" in unsafe_settings[section]:
                for k in ["secret"]:
                    env_key = f"AUTH_{k.upper()}"
                    if env_key in os.environ:
                        unsafe_settings[section]["auth"][k] = os.environ[env_key]
